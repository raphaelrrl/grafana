...
instalar_grafana(){
  ...
  grep -q 'FLOWSPEC-UMENU' "$IDX" || sed -i 's|</head>|<style>/*FLOWSPEC-UMENU*/ body.fs-viewer li:has(> a[href^="/profile"]), body.fs-viewer a[href^="/profile"], body.fs-viewer li:has(> a[href^="/dashboard/public"]), body.fs-viewer li:has(> a[href^="/dashboard/recently-deleted"]), body.fs-viewer li:has(> a[href^="/playlists"]), body.fs-viewer li:has(> a[href^="/library-panels"]), body.fs-viewer li:has(> a[href^="/dashboard/snapshots"]) {display:none !important;}</style>\n</head>|' "$IDX"
  # "Dashboards" -> "Abrir chamado" (site Flowspec) SO para Viewer; some com subitens
  grep -q 'FLOWSPEC-CHAMADO' "$IDX" || sed -i "s#</body>#<script>/*FLOWSPEC-CHAMADO*/(function(){var U=\"${CHAMADO_URL}\";function f(){if(!document.body.classList.contains(\"fs-viewer\"))return;document.querySelectorAll(\"nav a[href='/dashboards'], aside a[href='/dashboards'], nav a[href='/dashboards/'], aside a[href='/dashboards/']\").forEach(function(a){if(!a.dataset.fs){a.dataset.fs=\"1\";a.href=U;a.target=\"_blank\";a.rel=\"noopener\";}a.querySelectorAll(\"span,div\").forEach(function(e){if(e.children.length===0\&\&e.textContent.trim()===\"Dashboards\")e.textContent=\"Abrir chamado\";});if(a.children.length===0\&\&a.textContent.trim()===\"Dashboards\")a.textContent=\"Abrir chamado\";});document.querySelectorAll(\"[role='menu'] li, [role='menu'] a, [role='menu'] button\").forEach(function(el){var t=el.textContent.trim();if(/^(change theme|alterar tema|notification history|hist[oó]rico de notifica|profile|perfil)$/i.test(t)){var li=el.closest(\"li\")||el;li.style.setProperty(\"display\",\"none\",\"important\");}});}f();new MutationObserver(f).observe(document.body,{childList:true,subtree:true});document.addEventListener(\"click\",function(e){if(!document.body.classList.contains(\"fs-viewer\"))return;var a=e.target.closest\&\&e.target.closest(\"a[data-fs]\");if(!a)return;e.preventDefault();e.stopImmediatePropagation();window.open(U,\"_blank\",\"noopener\");},true);})();</script></body>#" "$IDX"
