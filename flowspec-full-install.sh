...
instalar_grafana(){
  ...
  sed -i 's|</head>|<style>/*FLOWSPEC-UMENU*/ body.fs-viewer li:has(> a[href="/profile"]), body.fs-viewer a[href="/profile"], body.fs-viewer li:has(> a[href^="/profile/notifications"]), body.fs-viewer a[href^="/profile/notifications"], body.fs-viewer li:has(> a[href^="/dashboard/public"]), body.fs-viewer li:has(> a[href^="/dashboard/recently-deleted"]), body.fs-viewer li:has(> a[href^="/playlists"]), body.fs-viewer li:has(> a[href^="/library-panels"]), body.fs-viewer li:has(> a[href^="/dashboard/snapshots"]) {display:none !important;}</style>\n</head>|' "$IDX"
  # "Dashboards" -> "Abrir chamado" (site Flowspec) SO para Viewer; some com subitens
  sed -i 's#<script>/\*FLOWSPEC-CHAMADO\*/[^<]*</script>##' "$IDX"
  sed -i "s#</body>#<script>/*FLOWSPEC-CHAMADO*/(function(){var U=\"${CHAMADO_URL}\";function f(){if(!document.body.classList.contains(\"fs-viewer\"))return;document.querySelectorAll(\"a[href^='/dashboards']\").forEach(function(a){if(a.dataset.fs)return;var c=a.cloneNode(true);c.dataset.fs=\"1\";c.href=U;c.target=\"_blank\";c.rel=\"noopener\";var w=document.createTreeWalker(c,NodeFilter.SHOW_TEXT),n;while((n=w.nextNode())){if(n.nodeValue.trim()===\"Dashboards\")n.nodeValue=\"Abrir chamado\";}c.addEventListener(\"click\",function(e){e.stopPropagation();});a.replaceWith(c);});document.querySelectorAll(\"[role='menu'] li, [role='menu'] a, [role='menu'] button\").forEach(function(el){var t=el.textContent.trim();if(/^(change theme|alterar tema|notification history|hist[oó]rico de notifica|profile|perfil)$/i.test(t)){var li=el.closest(\"li\")||el;li.style.setProperty(\"display\",\"none\",\"important\");}});}f();new MutationObserver(f).observe(document.body,{childList:true,subtree:true});})();</script></body>#" "$IDX"
