...
grep -q 'FLOWSPEC-UI' "$IDX" || sed -i 's|</head>|<style>/*FLOWSPEC-UI*/ body.fs-viewer [data-testid*="new share button"], body.fs-viewer [data-testid*="new export button"], body.fs-viewer [data-testid*="share-button"], body.fs-viewer button[aria-label="Help"], body.fs-viewer li:has(a[href^="/alerting"]), body.fs-viewer li:has(a[href^="/drilldown"]), body.fs-viewer li:has(a[href^="/bookmarks"]), body.fs-viewer li:has(a[href*="starred"]), body.fs-viewer *:has(> a[href^="/playlists"]), body.fs-viewer *:has(> a[href^="/library-panels"]), body.fs-viewer *:has(> a[href^="/dashboard/snapshots"]) {display:none !important;}</style>\n</head>|' "$IDX"
  ...
  sed -i "s#</body>#<script>/*FLOWSPEC-CHAMADO*/(function(){var U=\"${CHAMADO_URL}\";function f(){if(!document.body.classList.contains(\"fs-viewer\"))return;document.querySelectorAll(\"a[href='/dashboards'], a[href='/dashboards/']\").forEach(function(a){if(a.dataset.fs)return;var c=a.cloneNode(true);c.dataset.fs=\"1\";c.href=U;c.target=\"_blank\";c.rel=\"noopener\";c.querySelectorAll(\"span,div\").forEach(function(e){if(e.children.length===0\&\&e.textContent.trim()===\"Dashboards\")e.textContent=\"Abrir chamado\";});if(c.children.length===0\&\&c.textContent.trim()===\"Dashboards\")c.textContent=\"Abrir chamado\";c.addEventListener(\"click\",function(e){e.stopPropagation();});a.replaceWith(c);});document.querySelectorAll(\"[role='menu'] li, [role='menu'] a, [role='menu'] button\").forEach(function(el){var t=el.textContent.trim();if(/^(change theme|alterar tema|notification history|hist[oó]rico de notifica|profile|perfil)$/i.test(t)){var li=el.closest(\"li\")||el;li.style.setProperty(\"display\",\"none\",\"important\");}});}f();new MutationObserver(f).observe(document.body,{childList:true,subtree:true});})();</script></body>#" "$IDX"
